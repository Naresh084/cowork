...
```bash
pnpm --filter @gemini-cowork/core test -- src/agent.integration.test.ts
pnpm --filter @gemini-cowork/core typecheck
pnpm --filter @gemini-cowork/sidecar test -- src/ipc-handler.idempotency.test.ts src/agent-runner.release-gate.test.ts
