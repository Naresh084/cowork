...
export const SETTINGS_HELP_CONTENT: SettingHelpEntry[] = [
  ...
  },
  {
    ...
    description: 'Step 1 of guided setup. Selects which provider handles phone-to-desktop routing over internet.',
    what_it_controls:
      'Switches tunnel orchestration path between Tailscale, Cloudflare Tunnel, or custom endpoint mode and gates downstream setup steps.',
    when_to_use: 'Choose first, before domain/name/dependency/auth actions, so provider-specific setup is accurate.',
    tool_impact: ['mobile gateway', 'tunnel dependency/auth flow', 'pairing endpoint generation', 'remote websocket stream'],
    security_notes: 'Use managed secure tunnel modes for internet access and avoid exposing raw local ports directly.',
    requires_new_session: false,
  },
  {
    ...
    description:
      'Step-based orchestration for tunnel name/domain/private-public policy, dependency install, authentication, and start/stop.',
    what_it_controls:
      'Runs provider-aware lifecycle from UI: configuration apply, binary install, account auth (when required), and managed tunnel runtime.',
    when_to_use: 'Use this guided flow to complete full remote setup without terminal commands.',
    tool_impact: ['remote endpoint reachability', 'domain routing readiness', 'mobile pairing readiness'],
    security_notes:
      'Only keep tunnels running when needed; use private scope where possible and review endpoint/domain values before pairing.',
    requires_new_session: false,
  },
  {
    id: 'remote.pairingQr',
    title: 'Remote Pairing QR',
    description: 'Generates short-lived QR only after tunnel setup and endpoint verification are complete.',
    what_it_controls: 'Issues one-time pairing codes and device tokens scoped to remote API access after setup gates pass.',
    when_to_use: 'Use after tunnel startup succeeds, then scan in iOS/Android app to link device.',
