    ...
    if (activeTab === 'general') {
      return (
        <div className="space-y-4">
          ...
          >
            <div className="space-y-2 rounded-xl border border-white/[0.08] bg-black/20 p-3">
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm text-white/90">Stitch MCP API Key</p>
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  disabled={isAuthLoading || !stitchKeyDraft.trim()}
                  onClick={async () => {
                    try {
                  ...
                  className={cn(
                    'inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors',
                    isAuthLoading || !stitchKeyDraft.trim()
                      ? 'bg-white/[0.04] text-white/30 cursor-not-allowed'
                      : 'bg-[#1D4ED8] text-white hover:bg-[#3B82F6]',
                  )}
                >
                ...
                <button
                  type="button"
                  disabled={isAuthLoading || !stitchApiKey}
                  onClick={async () => {
                    try {
          ...
          <SectionCard
            title="Specialized Google Models"
            description="Configure non-media specialized models used by advanced tools."
          >
            <div className="space-y-2 rounded-xl border border-white/[0.08] bg-black/20 p-3">
              <div className="flex items-center justify-between gap-2">
              ...
              <p className="text-xs text-white/50">
                Used by browser automation and computer-use capabilities.
              </p>
              <input
                type="text"
            ...
            <div className="space-y-2 rounded-xl border border-white/[0.08] bg-black/20 p-3">
              ...
              <p className="text-xs text-white/50">
                Used by deep-research and long-horizon planning tasks.
              </p>
              <input
                type="text"
            ...
            <div className="flex items-center justify-between pt-1">
              <button
                type="button"
                onClick={handleResetSpecializedModels}
                disabled={!hasSpecializedModelChanges || isAuthLoading}
                className={cn(
                  'inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors',
                  !hasSpecializedModelChanges || isAuthLoading
                    ? 'bg-white/[0.04] text-white/30 cursor-not-allowed'
                    : 'bg-white/[0.06] text-white/75 hover:bg-white/[0.1]',
                )}
              >
